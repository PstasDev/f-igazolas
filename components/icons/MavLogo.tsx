import React from 'react';

interface MavLogoProps {
  className?: string;
  size?: number;
}

export const MavLogo: React.FC<MavLogoProps> = ({ 
  className = '', 
  size = 24
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 510 510"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
    >
      <g transform="translate(650 364.89)">
        {/* Yellow background */}
        <rect
          height="499.88"
          width="499.74"
          y="182.36"
          x="-644.87"
          fill="#fac65c"
        />
        {/* Blue M logo */}
        <path
          fill="#003b73"
          d="m-396.81 219.36c-33.339 0-62.443 18.036-78.066 44.854h-65.12l8.0633 29.995h135.12c8.3064 0 15.038 6.7138 15.038 14.998 0 8.2839-6.732 14.998-15.038 14.998h-127.06l8.0633 29.995h40.763c15.584 26.979 44.774 45.16 78.234 45.16 33.07 0 61.992-17.751 77.703-44.214h44.502l8.2865-30.802h-41.516c0.81905-4.8744 1.2555-9.8915 1.2555-14.998 0-5.1062-0.43648-10.124-1.2555-14.998h49.552l8.2865-30.802h-69.11c-15.711-26.455-44.634-44.186-77.703-44.186zm0 29.995c33.226 0 60.154 26.855 60.154 59.991s-26.928 60.019-60.154 60.019c-15.335 0-29.325-5.7406-39.954-15.165h39.954c24.919 0 45.115-20.141 45.115-44.993s-20.196-44.993-45.115-44.993h-39.619c10.583-9.2516 24.443-14.859 39.619-14.859z"
        />
        {/* Blue bottom section */}
        <rect
          height="249.75"
          width="500"
          y="432.61"
          x="-645"
          fill="#003b73"
        />
        {/* MÁV text */}
        <path
          fill="#fac65c"
          d="m-373.7 476.72-15.899 25.509h34.138l25.088-25.509zm-148.76 33.64-51.54 0.14v92.101h31.771v-71.868l19.129 71.98h28.235l19.129-71.98v71.868h31.771v-92.101l-51.541-0.14032-13.477 50.681zm126.81 0-38.398 92.353h34.444l4.7058-11.281h47.559l4.7058 11.281h34.444l-38.398-92.353zm67.709 0 38.426 92.353h35.112l38.398-92.353h-34.444l-21.524 51.747-21.496-51.747zm-43.177 23.881 14.841 35.667h-29.683z"
        />
        {/* Border */}
        <rect
          height="500"
          width="499.99"
          stroke="#000"
          y="182.48"
          x="-644.99"
          strokeWidth="4"
          fill="none"
        />
      </g>
    </svg>
  );
};

export default MavLogo;
